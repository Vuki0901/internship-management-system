...
public class DownloadInternshipReportPdfEndpoint : Endpoint<DownloadInternshipReportPdfRequest, DownloadInternshipReportPdfResult>
{
    ...
    {
        Get("internship-supervisors/internships/{internshipId}/report/download");
        Roles(nameof(Domain.InternshipSupervisor), nameof(Administrator), nameof(Student), nameof(Mentor));
    }
    ...
    public override async Task HandleAsync(DownloadInternshipReportPdfRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            studentUser = await _databaseContext.Users
                .Include(u => u.Roles)
                .FirstOrDefaultAsync(u => u.Roles.Any(r => r is Domain.Student && r.Id == internship.StudentId), cancellationToken);
        }
